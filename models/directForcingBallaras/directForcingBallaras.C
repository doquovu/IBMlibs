/*---------------------------------------------------------------------------*\
       ...
       ||  //    \\/  |//       IBMlibs - directForcingBallaras class
       +===   \===\\ =//        OpenFOAM 5.0 - 13/4/2018
\*---------------------------------------------------------------------------*/

#include "directForcingBallaras.H"
...
namespace Foam
{
	defineTypeNameAndDebug(directForcingBallaras, 0);
	addToRunTimeSelectionTable(IBTechnique, directForcingBallaras, dictionary);
}

//---------------------------Private Member Functions------------------------//
Foam::vector Foam::directForcingBallaras::linearInterpolate
(
    const point p1,
    const point p2,
    const point target,
...
Foam::vector Foam::directForcingBallaras::bilinearInterpolate
(
...
Foam::vector Foam::directForcingBallaras::trilinearInterpolate
(
    const pointVectorField& phi,
    const point& target,
    const labelList& sourceLabel
...
void Foam::directForcingBallaras::makeIbForce
(
    volVectorField& U, 
    volVectorField& f
)
...
void Foam::directForcingBallaras::makeIbForceNew
(
    volVectorField& U, 
    volVectorField& f
)
{
    ...
    forAll(objects(), i)
    {
        if (objects()[i].objectType() == "IBSTL")
        {
            ...
            const labelListList& vtnc = stlObject.vtNeiCells();
            ...
            forAll(vtp, i)
            {
                ...
                vector Uvt(vector::zero);
                //- #1: Using trilinear interpolation from vertices of cell 
                //  that enclose the virtual point
                const labelList& curCellPoint = mesh_.cellPoints()[vtpc[i]];
                Uvt = trilinearInterpolate(Up, vtp[i], curCellPoint);
                
                //- #2: Using window function from neighbouring cells of cell
                //  that enclose the virtual point
                // forAll(vtp, i)
                // {
                //     forAll(vtnc[i], j)
                ...
                //                 vtp[i]
                //             );
                //         Uvt += U[vtnc[i][j]]*deltaFuncValue*dV();
                //     }
                // }
...
//---------------------------------Constructors------------------------------//
Foam::directForcingBallaras::directForcingBallaras
(
	// const word& name,
...
Foam::volVectorField Foam::directForcingBallaras::ibForce(volVectorField& U)
{
    ...
    (
        ...
        dimensionedVector("zero",
                dimensionSet(0,1,-2,0,0,0,0), vector::zero)
    );   
    if ( nObjects() != 0)
    {
...
Foam::volVectorField Foam::directForcingBallaras::ibForceInt()
{
...
Foam::volVectorField Foam::directForcingBallaras::ibForceInt(const volVectorField& rhs)
{
    Info<< "IBM: Calculating ibForceInt..."<<endl;
...
void Foam::directForcingBallaras::multiDirectForcing
(
    volVectorField& u,
    volVectorField& ibForce_
)
...
void Foam::directForcingBallaras::update()
{
...
void Foam::directForcingBallaras::write()
{
    for(int objI=0; objI<nObjects(); objI++)
    {
